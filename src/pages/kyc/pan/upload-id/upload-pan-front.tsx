...
export interface KYCUploadPanFrontProps {}
const KYCUploadPanFront: FC<KYCUploadPanFrontProps> = () => {
  const { t } = useTranslation("KYC");
  const navigate = useNavigate();
  const { idFrontImage } = useOwnerStore();

  return (
    <KYCScreen
      className="relative"
      onAction={() => navigate("/kyc/pan/capture-front")}
      ...
      footer={
        <>
          <ButtonPrimary onClick={() => navigate("/kyc/pan/capture-back")} className="w-full" size={"medium"}>
    ...
    >
      <div className="w-[328px] h-[232px] absolute left-[50%] top-[186px] transform -translate-x-1/2">
        ...
        <div className="w-[289px] h-[185px] bg-[#000] absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2">
          <img src={idFrontImage || ''} alt="pan_photo" className="text-white" />
        </div>
      </div>
      ...
      <div className="w-[244px] h-[44px] flex justify-center items-center absolute left-1/2 bottom-[70px] transform -translate-x-1/2">
        <p
          ...
          onClick={() => navigate("/kyc/pan/capture-front")}
        >
          Repick Photo
        </p>
      </div>
...
export default KYCUploadPanFront;
