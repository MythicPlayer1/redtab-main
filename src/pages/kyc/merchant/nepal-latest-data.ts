...
export const getAllProvinces = (NepalList: any []) => {
    NepalList?.map((province) => {
        return province;
    }
    );
...
export function getAllDistricts(NepalList: any[], provinceName: string) {
    const province = NepalList.find((p) => p.name === provinceName);
    if (province) {
...
getAllDistricts(addressObject, "BAGMATI PROVINCE");


export function getAllMunicipalities(NepalList: any[],province:string, districtName: string) {
    const provinceData = NepalList.find((p) => p.name === province);
    if (provinceData) {
        const district = provinceData.districtList.find((d:any) => d.name === districtName);
        if (district) {
            district.municipalityList.forEach((municipality: any) => {
                return municipality;
            });
        } else {
            return [];
