...
// create a method that reutrns the list of provinces
export const getAllProvinces = (NepalList: any[]): string[] => {
  return NepalList?.map((province) => province.name) || [];
};


export function getAllDistricts(NepalList: any[], provinceName: string) {
  const province = NepalList.find((p) => p.name.toLowerCase() === provinceName.toLowerCase());

  if (province) {
     return province.districtList.map((district: any) =>district.name.toLowerCase());
  } else {
...
export function getAllDistrictsNamesonly(NepalList: any[], provinceName: string) {
  const province = NepalList.find((p) => p.name.toLowerCase() === provinceName.toLowerCase());
  if (province) {
     return province.districtList.map((district: any) =>district.name.toLowerCase());
  } else {
...
export function getAllPalikas(NepalList: any[], province: string, districtName: string) {
  const provinceData = NepalList.find((p) => p.name === province);
  if (provinceData) {
    const district = provinceData.districtList.find((d: any) => d.name.toLowerCase() === districtName.toLowerCase());
    if (district) {
       return district.municipalityList.map((municipality: any) =>municipality.name);
    } else {
      console.log(`District ${districtName} not found`);
      return [];
    }
  } else {
    console.log(`Province ${province} not found`);
    return [];
  }
}
