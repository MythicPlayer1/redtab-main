...
export const InputCreatePasswordStep: FC<PropsWithChildren<InputCreatePasswordStepProps>> = (props) => {
  ...
  return (
    <Layout
      title={t('letGetStarted', { defaultValue: "Create password" })}
      subTitle={t('enterPassword', { defaultValue: 'Minimum 8 character' })}

      footer={<>
        <ButtonPrimary disabled={!password} className="w-full" size="large" onClick={() => props.onSubmit?.({ password })}>{t('createPassword', { defaultValue: 'Create' })}</ButtonPrimary>
      </>}>
