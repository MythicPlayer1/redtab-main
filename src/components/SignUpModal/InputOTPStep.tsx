...
export const InputOTPStep: FC<PropsWithChildren<InputOTPStepProps>> = (props) => {
  ...
  return (
    <Layout
      ...
      footer={<>
        <ButtonSecondary onClick={() => props.onAction?.('change')} className="w-full" size="large">{t('changeNumber', { defaultValue: 'Change number' })}</ButtonSecondary>
        <ButtonPrimary disabled={!otp} className="w-full" size="large" onClick={() => props.onSubmit?.({ otp })}>{t('resendCode', { defaultValue: 'Resend Code' })}</ButtonPrimary>
      </>}>
