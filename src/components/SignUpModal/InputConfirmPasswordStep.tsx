...
export interface InputConfirmPasswordStepProps {
...
export const InputConfirmPasswordStep: FC<PropsWithChildren<InputConfirmPasswordStepProps>> = (props) => {
  ...
  return (
    <Layout
      title={t('letGetStarted', { defaultValue: "Confirm password" })}
      subTitle={t('enterPassword', { defaultValue: 'Minimum 8 character' })}

      footer={<>
        <ButtonPrimary disabled={!password} className="w-full" size="large" onClick={() => props.onSubmit?.({ password })}>{t('confirmPassword', { defaultValue: 'Confirm' })}</ButtonPrimary>
      </>}>
