...
export const InputBase = forwardRef(
  <T,>({ Input, inputProps, focusWithinBorder, ...props }: InputBaseProps<T>, ref: Ref<HTMLInputElement>) => {
    return (
      <div className={`${props.errorMessage !== "" ? "mb-0" : "mb-4"} font-poppins`}>
        <div
          className={`${
            focusWithinBorder !== "none" ? "focus-within:border-b-[#EA4335]" : ""
          } gap-2 w-full min-h-[3.75rem] py-[0.3rem] border-b-[1px] border-b-[#EAECF0] capitalize border-[transparent] flex items-center justify-between`}
        >
          {props.prependOn}
          <div className="grow">
            {props.label !== false && (
              <label className={clsx("text-xs text-[#98A2B3] font-normal tracking-wide", { hidden: !props.value })}>
                {props.label}
              </label>
            )}
            <Input
        ...
        {props.errorMessage !== "" && (
          <p className="pt-[1px] w-full text-sm tracking-wide text-primaryColor">{props.errorMessage}</p>
        )}
      </div>
    );
  }
