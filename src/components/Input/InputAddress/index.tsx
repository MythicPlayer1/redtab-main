...
import { addressObject, getAllDistricts, getAllProvinces } from "../../../pages/kyc/merchant/nepal-data";
...
const MapOption: FC<MapOptionProps> = (props) => {
  return (
    <div className="flex p-3 items-center hover:bg-secondaryColor" onClick={() => props.onClick?.()}>
      <div className="w-12 pl-2 pr-5">{props.icon}</div>

      <div>
        <div className="font-semibold text-sm text-[#1D2939]">{props.title}</div>
...
const MapListOption: FC<MapOptionProps> = (props) => {
  ...
  const geocodeByPlaceId = (placeId: string) => {
    ...
    geocoder.geocode({ placeId: placeId }, (results, status) => {
      ...
      if (status === "OK" && results && results[0]) {
        ...
        routes("/kyc/merchant/confirm-location");
      } else {
  ...
  return (
    <div className="flex p-2 items-center hover:bg-secondaryColor" onClick={handleClick}>
      ...
      <div>
        <div className="text-[#475467] text-sm">{props.description}</div>
        {/* <div className='text-[#475467] text-xs'>{props.description}</div> */}
      </div>
    </div>
  );
...
const MapSelection: FC<MapSelectionProps> = (props) => {
  ...
  return (
    <>
      ...
      <div className="fixed top-0 left-0 bottom-0 right-0 font-poppins">
        <div
...
export const InputAddress: FC<InputAddressProps> = (props) => {
  ...
  return (
    <>
      <button
        ...
        className="gap-2 w-full  min-h-[3.75rem] py-[0.3rem] border-b-[1px] border-b-[#EAECF0] border-[transparent] focus-within:border-b-[#EA4335] flex items-center justify-between"
      >
        <p className="text-[#7A7E83]">Enter Address</p>
        <svg
          onClick={() => setIsModalVisible(true)}
